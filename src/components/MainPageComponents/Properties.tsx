...
export default function Properties({
  ...
  minPrice,
  maxPrice,
}: {selectedRegions: string[], minPrice: number | null, maxPrice: number | null}) {
  return(
    <div className="absolute flex top-[240px] left-[162px] gap-[16px]">
      ...
      <div className={`${selectedRegions.length > 0 ? 'flex' : "hidden"} items-center justify-center cursor-pointer border-[1px] rounded-[43px] gap-[4px] py-[6px] px-[10px] text-[14px]`}>
          {selectedRegions.map((region, index) => (
            <div key={index}>{region}{index < selectedRegions.length - 1 && ', '}</div> // Correct comma handling
          ))} 
          <IoIosClose />
        </div>

        {/* Other filters (area, price, bedrooms) can be added similarly */}
        ...
        <div className="flex items-center justify-center cursor-pointer border-[1px] rounded-[43px] gap-[4px] py-[6px] px-[10px] text-[14px]">{minPrice}₾ - {maxPrice}₾ <IoIosClose /></div>
