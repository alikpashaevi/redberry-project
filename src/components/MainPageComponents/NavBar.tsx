...
export default function NavBar() {
  ...
  const [onSelectRegions, setSelectRegions] = useState<string[]>([]);
  ...
  return (
    <nav className="absolute top-[177px] left-[162px] flex justify-between align-center w-[1596px] h-auto">
      <div className="flex p-[6px] gap-[24px] rounded-[10px]  border-[1px] border-[#DBDBDB]">
        <div className="flex items-center cursor-pointer gap-[4px] py-[8px] px-[14px] text-[#021526] font-medium text-[16px]" onClick={toggleModal}>რეგიონი <IoIosArrowDown /></div>
        <div className="flex items-center cursor-pointer gap-[4px] py-[8px] px-[14px] text-[#021526] font-medium text-[16px]">საფასო კატეგორია <IoIosArrowDown /></div>
        <div className="flex items-center cursor-pointer gap-[4px] py-[8px] px-[14px] text-[#021526] font-medium text-[16px]">ფართობი <IoIosArrowDown /></div>
      ...
      <RegionModal  
        isOpen={isOpen}
        onSelectRegions={setSelectRegions}
      />
    </nav>
  )
}